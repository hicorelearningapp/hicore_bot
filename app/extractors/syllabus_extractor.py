...
class SyllabusExtractor:
    """Extract structured syllabus data from raw document text."""
    ...
    def _extract_field_raw(self, start_tag: str, end_tag_pattern: str) -> Optional[str]:
        ...
        # fallback: content until end-of-text
    ...
    def _parse_course_outcomes(self, raw_outcomes: str) -> List[Dict[str, str]]:
        ...
        for i in range(1, len(outcomes) - 1, 2):
            ...
            desc_match = re.search(r'Description:\s*(.*?)\s*- Mapping:', co_text_raw,
                                   re.DOTALL | re.IGNORECASE)

            if desc_match:
                description = desc_match.group(1).strip()
                mapping = re.search(r'Mapping:\s*(.*)', co_text_raw,
                                     re.DOTALL | re.IGNORECASE)
            ...
            outcomes_list.append({
                ...
                "Mapping": re.sub(r'\s+', ' ', mapping_text)
            })
    ...
    def _parse_unit_details(self, raw_units: str) -> List[Dict[str, str]]:
        ...
        units_list = []
        unit_splits = re.split(
            r'---\s*(?:Unit |CO)?\s*([IVXLCDM\d]+)\s*\(.*?\)\s*---',
            raw_units,
            flags=re.IGNORECASE
        )
        unit_splits = unit_splits[1:]  # remove prefix
    ...
    def _parse_learning_resources(self, raw_resources: str) -> Dict[str, List[str]]:
        ...
        text_books_raw = re.search(
            r'TEXT BOOKS:\s*(.*?)\s*(Reference Books:|Web Resources:|$)',
            raw_resources, re.DOTALL | re.IGNORECASE
        )
        if text_books_raw:
            books = re.split(r'\s*[\▪\-\u2022]\s*', text_books_raw.group(1).strip())
            resources_dict["TEXT BOOKS"] = [self._clean_value(b) for b in books if b.strip()]

        ref_books_raw = re.search(
            r'Reference Books:\s*(.*?)\s*(Web Resources:|$)',
            raw_resources, re.DOTALL | re.IGNORECASE
        )
        if ref_books_raw:
        ...
        web_raw = re.search(r'Web Resources:\s*(.*)', raw_resources, re.DOTALL | re.IGNORECASE)
        if web_raw:
            urls = [url.strip() for url in re.split(r'\s+', web_raw.group(1)) if url.startswith("http")]
    ...
    def _extract_overall_objectives(self, unit_details: List[Dict[str, str]]) -> str:
        ...
        return combined or 'Overall Course Objectives not explicitly listed.'
    ...
    def _process_document_to_key_value_pairs(self) -> Dict[str, Any]:
        ...
        kb["Credits"] = self._clean_value(
            self._extract_field_raw("Credits:", r'Inst\. Hours')
        ) or 'N/A'

        kb["Inst. Hours (LTP)"] = self._extract_field_raw(
            "Inst. Hours", r'Marks Breakdown'
        ) or 'N/A'

        kb["CIA Marks"] = self._clean_value(
            self._extract_field_raw("CIA Marks:", r'External Marks:')
        ) or 'N/A'

        kb["External Marks"] = self._clean_value(
            self._extract_field_raw("External Marks:", r'Total Marks:')
        ) or 'N/A'

        kb["Total Marks"] = self._clean_value(
            self._extract_field_raw("Total Marks:", r'Course Outcomes')
        ) or 'N/A'
        ...
        raw_units = self._extract_field_raw(
            "Unit Details",
            r'Overall Course Objectives|Evaluation and Assessment|Learning Resources'
        )
        kb["Unit Details"] = self._parse_unit_details(raw_units)

        raw_obj = self._extract_field_raw(
            "Overall Course Objectives", r'Evaluation and Assessment|Learning Resources'
        )
        kb["Overall Course Objectives"] = raw_obj or self._extract_overall_objectives(kb["Unit Details"])

        kb["Evaluation (Internal)"] = self._extract_field_raw(
            "Methods of Evaluation (Internal):",
            r'Methods of Assessment'
        ) or 'N/A'

        kb["Evaluation (External)"] = self._extract_field_raw(
            "Methods of Assessment (External):",
            r'Mapping with Programme Outcomes:'
        ) or 'N/A'
